...
kc_lookup_username() {
  ...
  userid=`echo $result | grep -Eo '"id":.*?[^\\]"' | cut -d':'  -f 2 | sed -e 's/"//g'| sed -e "s/,createdTimestamp//g" `
  # echo $result
  # echo $userid
  echo $userid | sed -e "s/,createdTimestamp//g"
  msg="$username: lookup "
  process_result "200" "$result" "$msg"
  return $? #return status from process_result
...
echo $base_url/admin/realms/$realm/users/$userid/groups/$groupid
...
kc_set_pwd() {
  ...
  msg="$base_url/admin/realms/$realm/users/$userid/reset-password : $username: password set to $password"
  process_result "204" "$result" "$msg"
  return $? #return status from process_result
}
